...
import numpy
from scipy.ndimage import *  ## imports everything from ndimage and overwrites functions we need to fix
...
def sum(input, labels=None, index=None):
    ...
    # this bug is fixed in scipy 0.8.0
    ...
    if index is not None:
        _fix_dtype(index)
    # fixing a change of behavior introduced in scipy 0.8.0 
    # when the labels is None while index is not None
    # e.g. compute zone.number_of_agents(household) when all households are unplaced
